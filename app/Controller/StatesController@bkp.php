...
**/
    public function admin_list($search=null){
		...
		$conditions=array();
		@$page=($this->params->named['page'])?$this->params->named['page']:'';
		if(!empty($page)){
		...
		if(!empty($this->params->query['search'])){
			$this->Session->write('param',$this->params->query['search']);
	    ...
	    $chk_unique = $this->State->find('count' , array('conditions'=>array('State.name' => $this->request->data['State']['name'])));
		 ...
		 if($chk_unique == 0){
			 if($this->State->save($this->request->data , false)){
				...
				$search=$this->Session->read('param');
				$this->redirect(array('controller'=>'states','action'=>'admin_list','page'=>$page,'?'=>array('search'=>$search)));
			 }
		 }else{
		...
		if(empty($this->request->data)){
			...
			if(!empty($this->request->data)){
				if($this->State->save($this->request->data)){
					...
					$params=$this->Session->read('param');
					$this->redirect(array('controller'=>'states','action'=>'list','admin'=>true,'?'=>array('search'=>$params)));
				}
			}
		}
	} 
